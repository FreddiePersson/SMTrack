...
for outFrame = 1 : data.DETECT.stackSize
    ...
    if numPoints>0
        ...
        for ind2=1:numel(stats)
            coord=stats(ind2).WeightedCentroid;
            posFrame(round(coord(2)), round(coord(1))) = ind2;
        end
...
DETECT = struct('full_filename', full_filename, 'rawData', finalStack(:, :, 1:col*2-2), 'timeBetweenFrames', num2str(data.DETECT.timeBetweenFrames), 'relIntTh', num2str(1),...
