...
uicontrol(...
    ...
    'Style','text',...
    'Units','pixels',...
    'BackgroundColor',get(findobj('Tag','hANA'),'Color'),...
    'FontSize',40,...
...
firstTrajFrame = 0;
traj = cell(1, max(frameNr)*200);%floor((max(frameNr)-min(frameNr))/2));
lastTrajNr = 0;

nextHash = zeros(10000,1);
...
if nargin>1
    baseName = get(findobj('Tag', 'ANALYZE_Locset_sendToWS_1'), 'String');
    name = [baseName '_Traj_Pos' num2str(varargin{1})];
    assignin('base', name, finalTraj);
else
